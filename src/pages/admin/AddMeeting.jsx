...
const AddMeeting = () => {
  ...
  const fetchMeetingData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/meetings/no-follow-date`,
        { headers }
      );
      if (!response.ok) {
              ...
              currentMeetings.map((meeting, index) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 text-gray-900 font-medium truncate">
                    {meeting.companyName?.length > 25
                      ? `${meeting.companyName.slice(0, 25)}...`
                      : meeting.companyName || "—"}
                  </td>
                  ...
                  <td className="py-3 px-4 text-gray-700 truncate">
                    {meeting?.person?.assignedStaff?.username || "—"}
                  </td>

                  {/* Date & Time */}
                  <td className="py-3 px-4 text-center text-gray-700 truncate">
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 border rounded-lg border-gray-400 p-4">
                <div>
