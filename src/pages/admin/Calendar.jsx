...
import { FiChevronLeft, FiChevronRight, FiCalendar, FiClock, FiUser, FiPlus, FiX, FiSearch } from 'react-icons/fi';
...
const Calendar = () => {
    ...
    for (let i = 0; i < 6; i++) {
      for (let j = 0; j < 7; j++) {
        if ((i === 0 && j < firstDay) || dayCount > daysInMonth) {
          ...
          const isToday = isSameDay(new Date(currentDate.getFullYear(), currentDate.getMonth(), dayCount), new Date());
          const hasEvents = getEventsForDate(new Date(currentDate.getFullYear(), currentDate.getMonth(), dayCount)).length > 0;
  ...
  const renderMainCalendar = () => {
          ...
          <div className="space-y-3">
            {dayEvents.length > 0 ? (
              dayEvents.map((event, index) => (
                <div key={event.id} className="bg-blue-50 p-4 rounded-xl border border-blue-100 hover:shadow-md transition-all">
                  <div className="font-medium text-gray-900 flex items-center gap-2">
                    <FiUser className="text-blue-500" /> {event.title}
                  </div>
                  <div className="text-sm text-gray-600 flex items-center gap-2 mt-1">
                    <FiClock className="text-blue-500" /> {event.startTime} - {event.endTime}
                  </div>
                  {event.description && (
                    <div className="text-sm text-gray-600 mt-2">{event.description}</div>
                  )}
                  {event.participants && (
                    <div className="text-sm text-gray-600 mt-1">With: {event.participants}</div>
                  )}
                </div>
              ))
            ) : (
          ...
          <div className="grid grid-cols-7 gap-2">
            {Array.from({ length: 7 }).map((_, i) => {
              ...
              return (
                <div 
                  key={i} 
                  className={`rounded-xl p-3 border transition-all ${isSelected ? 'bg-newPrimary/10 border-newPrimary' : 'bg-white border-gray-100'} ${isToday ? 'ring-2 ring-blue-100' : ''}`}
                  onClick={() => setSelectedDate(day)}
                >
                  <div className="font-medium text-gray-500 text-sm">{days[day.getDay()]}</div>
                  <div className={`text-lg font-bold my-1 ${isToday ? 'text-newPrimary' : 'text-gray-800'}`}>{day.getDate()}</div>
                  <div className="mt-2 space-y-1">
                    {dayEvents.slice(0, 3).map((event, j) => (
                      <div key={event.id} className={`text-xs p-1.5 rounded-lg truncate ${
                        j % 2 === 0 ? 'bg-blue-100 text-blue-800' : 'bg-pink-100 text-pink-800'
                      }`}>
                        {event.title}
                      </div>
                    ))}
    ...
    for (let i = 0; i < 6; i++) {
      for (let j = 0; j < 7; j++) {
        if ((i === 0 && j < firstDay) || dayCount > daysInMonth) {
          calendarDays.push(<div key={`empty-${i}-${j}`} className="h-24 border border-gray-100 rounded-lg bg-gray-50"></div>);
        } else {
          ...
          calendarDays.push(
            <div
              ...
              className={`h-24 border border-gray-100 p-2 rounded-lg transition-all ${
                isSelected ? 'bg-newPrimary/10 border-newPrimary' : 'bg-white hover:bg-gray-50'
              } ${isToday ? 'ring-2 ring-blue-100' : ''}`}
              onClick={() => handleDateClick(dayCount)}
            >
              <div className={`text-right font-medium ${isToday ? 'text-newPrimary' : 'text-gray-700'}`}>{dayCount}</div>
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-newPrimary">Calendar</h1>
              ...
              {filteredEvents.slice(0, 3).map(event => (
                <div key={event.id} className="bg-blue-50 p-3 rounded-lg border border-blue-100">
                  ...
                  <div className="text-xs text-gray-500 flex items-center gap-1 mt-1">
                    <FiClock size={12} /> 
                    {months[event.date.getMonth()]} {event.date.getDate()}, {event.startTime} - {event.endTime}
                  </div>
                </div>
              ))}
              ...
              {['day', 'week', 'month'].map((mode) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-sm rounded-md transition-all ${
                    viewMode === mode ? 'bg-white shadow-sm text-newPrimary font-medium' : 'hover:bg-gray-200 text-gray-600'
                  }`}
